...
TEST_RESPONSE_STORE = {
    "test-service1": {
        ...
        "test-region2": {"test-operation1": "string"},
    },
    "test-service2": {"test-region1": {"test-operation1": "string"}},
}
for i in range(3, 103):
    TEST_RESPONSE_STORE.update(
        {"test-service" + str(i): {"test-region1": {"test-operation1": "string"}}}
    )

TEST_EXCEPTION_STORE = {
    "test-service": {"test-operation1": {"test-region": "exception"}}
}
